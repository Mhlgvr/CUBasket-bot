...
about_tournament = '''CU Backetball Cup – внутренний турнир среди студентов и сотрудников Центрального университета, организованный баскетбольным клубом ЦБК. Вступай в команду своего потока или приходи болеть за друзей!
...
Плей-офф будет отыгран с ¼ стадии (все команды выходят из группы). У команд будет минимум 2 игровых дня, максимум 3. 
...
Матчи 15 минут на групповом этапе и 22 минуты в плей-офф с остановками времени при штрафных бросках и таймаутах (перерывы будут на 7.5 минутах и на 11 минутах соответственно), формат 3х3 на два кольца, при бросковом фоле штрафные броски предусмотрены или команда в матче может взять 2 буллита за игру, 
Очки считаются по системе 1/2, у каждой команды на игру 2 таймаута. В случае равного счёта время продлевается. 
Судейство не предусмотрено, фолы регулируются командами самостоятельно, неспортивное поведение регулируется организаторами.
...
async def reg_2(message: Message, state: FSMContext):
    ...
    await message.answer('Выбери поток: ', reply_markup=await kb.teams())
...
async def add_thread_1(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text('SEND THREAD AND TEAM NAME SEPARATED BY _', reply_markup=kb.back3)
